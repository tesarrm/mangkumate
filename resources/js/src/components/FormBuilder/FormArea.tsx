...
export const FormArea: React.FC<FormAreaProps> = ({
    ...
    return (
        <div
            ref={sectionRef}
            // style={{ marginBottom: '32px', border: '1px solid #ccc', padding: '16px', position: 'relative' }}
...
const ColumnComponent: React.FC<ColumnComponentProps> = ({
    ...
    // const handleAddElement = (selectedOption) => {
    //     if (selectedOption) {
    ...
    return (
        <div
            ref={columnRef}
            // style={{ flex: 1, border: '1px dashed #ccc', padding: '16px', minHeight: '200px', position: 'relative' }}
            style={{ flex: 1, border: '1px dashed #ccc', padding: '16px', position: 'relative' }}
            // className="bg-[#f4f4f4] dark:bg-white-dark/20 p-3 pb-5 rounded-md mb-5 cursor-move"
            className="bg-[#f4f4f4] dark:bg-white-dark/20 p-3 pb-5 rounded-md mb-5 cursor-move overflow-auto"
            onMouseEnter={() => setIsHovered(true)}
            onMouseLeave={() => setIsHovered(false)}
            // onClick={() => onSelectColumn(column)} // Pilih column saat diklik
            onClick={(e) => {
                e.stopPropagation(); // Hentikan event bubbling
        ...
        >
            ...
            <div className="space-y-4">
                ...
                {/* Tombol "Add Element" */}
                {/* <div>
                    <button onClick={() => setShowDropdown(!showDropdown)} className="btn btn-sm bg-white shadow-none">
                        Add Element
                    </button>
                    ...
                    {showDropdown && (
                        <div style={{ marginTop: '8px', width: '200px' }}>
                            <Select
                                ...
                                // menuPlacement="top" // Menampilkan opsi di atas input
